...
{
    use ApiHelper; // <---- Использование трейта apiHelpers
    ...
    {$token = auth()
        ...
        ->plainTextToken; // Получение токена из сессии

        if (!$token) {
            // перенаправить на страницу авторизации или запросить токен
            return redirect()->route('get.token.from.auth');
        }
        try {
        ...
        $posts = DB::table('posts')->get();
            ...
            // Обработка ошибок
            return redirect()->route('error')->with('error', $e->getMessage()); // Или другое логирование
        }
